    ...
    <div class="bottom-layout">
      ...
      <div class="global-alert-panel">
        <div class="alert-header">
          <h3><i class="fa-solid fa-clipboard-list"></i> 全区预警与风险日志 </h3>
        ...
        <div class="alert-list">
          ...
          <div v-for="item in globalAlerts" :key="item.id"
               :class="['alert-item', getAlertCardClass(item.type)]">
            <div class="alert-main">
              ...
              <div class="alert-content" v-html="formatAlertMessage(item.wellId, item.message, item.type)">
              </div>
            </div>
...
const unhandledGroups = ref(0) // 预留给未来的井组预警
...
// ==========================================
// 🌟 核心分发策略：为未来扩展预留的标准化接口
// ==========================================
// 0 = 实时异常, 1 = 预测风险, 2 = 井组预警 (预留)

const getAlertCardClass = (type) => {
  if (type === 1) return 'alert-type-risk'
  if (type === 2) return 'alert-type-group' // 未来井组的紫色左边框
  return 'alert-type-anomaly'
}

const getTypeTagClass = (type) => {
  if (type === 1) return 'type-tag tag-risk'
  if (type === 2) return 'type-tag tag-group' // 未来井组的紫色 Tag
  return 'type-tag tag-anomaly'
}

const getTypeName = (type) => {
  if (type === 1) return '预测风险'
  if (type === 2) return '井组预警'
  return '实时异常'
}

const getTypeIcon = (type) => {
  if (type === 1) return 'fa-solid fa-eye'
  if (type === 2) return 'fa-solid fa-network-wired' // 网络拓扑图标
  return 'fa-solid fa-triangle-exclamation'
}

const formatAlertMessage = (wellId, msg, type) => {
  let color = '#e53e3e' // 默认红
  if (type === 1) color = '#e67e22' // 预测橙
  if (type === 2) color = '#9b59b6' // 井组紫
...
const fetchRealAlerts = async () => {
  try {
    ...
    // unhandledGroups.value = rawList.filter(l => l.warningType === 2 && l.status === '未处理').length // 未来解开即可

    globalAlerts.value = rawList.slice(0, 50).map(log => {
      ...
      return {
        ...
        type: log.warningType || 0 // 将 type 存入前端对象
      }
    })
  } catch (error) {
...
const updateChartOption = () => {
  ...
  lineChart.setOption({
    ...
    graphic: wellData.isOnline ? [] : [{
      ...
      style: { fill: '#94a3b8', text: '⚠ 等待传感器信号接入...', font: 'bold 22px sans-serif' }
    }],
    series: [
...
/* 🌟 预留：井组预警专属卡片渐变 (紫色) */
.card-group { background: linear-gradient(135deg, #c2e59c 0%, #64b3f4 100%); }
...
/* 🌟 多元化状态左边框颜色定义 */
.alert-type-anomaly { border-left: 4px solid #e53e3e; } /* 实时红 */
.alert-type-risk { border-left: 4px solid #e67e22; background: #fffaf0;}    /* 预测橙 (背景带微橙) */
.alert-type-group { border-left: 4px solid #9b59b6; background: #fdf4ff;}   /* 预留井组紫 */
